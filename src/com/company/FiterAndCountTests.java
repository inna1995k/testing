...
public class FiterAndCountTests {
    ...
    private static void testFilterAndCount() {
        ...
        People maximumAge = people.stream().max((p1,p2) -> p1.getAge().compareTo(p2.getAge())).get();
        System.out.println("maximumAge " + maximumAge);

        // Отсортировать по имени в обратном алфавитном порядке
        Collection<People> byname = people.stream().sorted((o1,o2) -> o1.getName().compareTo(o2.getName())).collect(Collectors.toList());
